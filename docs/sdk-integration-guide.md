...
└─────────────────────────┼────────────────────────────────────┘
        ...
        │  │ • 70% to Synora platform      │   │
        │  └──────────────────────────────┘   │
        │                                      │
        └─────────────────────────────────────┘
```

### What Synora Collects
...
The Synora SDK is distributed via our global CDN to ensure fast, reliable downloads during your firmware build process. Each release includes cryptographic checksums (SHA-256) to verify integrity before flashing to production devices.
...
The Synora SDK follows semantic versioning:

- **MAJOR version** (e.g., 2.0.0): Breaking API changes, new C ABI, incompatible with prior versions. Manufacturers must recompile and retest. Synora will publish a migration guide.
...
This example shows a complete `CMakeLists.txt` for a TV application that links the Synora SDK:
...
# Paths to Synora SDK
set(ACR_SDK_PATH "${CMAKE_SOURCE_DIR}/external/acr-sdk/v1.0.0" CACHE PATH "Synora SDK path")
...
if(NOT EXISTS "${ACR_LIB_PATH}")
    message(FATAL_ERROR "Synora SDK not found at ${ACR_LIB_PATH}")
endif()
...
# Link Synora SDK and dependencies
...
For Android TV apps, integrate the SDK via the Android NDK. The Synora team provides a Kotlin wrapper that bridges JNI to the native C library.
...
dependencies {
    // Synora SDK Kotlin wrapper (published to Maven Central)
...
<manifest xmlns="http://tizen.org/ns/packages" 
    ...
    <!-- Required privileges for Synora SDK -->
...
int main() {
    ...
    if (ret != ACR_OK) {
        fprintf(stderr, "Failed to initialize Synora: %d\n", ret);
        return 1;
    }
    ...
    if (user_consented) {
        ret = acr_start();
        if (ret != ACR_OK) {
            fprintf(stderr, "Failed to start Synora: %d\n", ret);
            return 1;
        }
    }
...
static void signal_handler(int sig) {
    if (g_acr_handle != NULL) {
        fprintf(stderr, "Shutting down Synora SDK gracefully...\n");
...
int main(int argc, char *argv[]) {
    ...
    printf("=== Synora SDK Integration Example ===\n");
    ...
    printf("Synora SDK initialized successfully\n");
    ...
    if (user_consented) {
        ...
        printf("Synora audio capture started\n");
    } else {
        printf("Synora capture skipped (user opted out)\n");
    }
...
void on_user_consent_changed(bool new_consent) {
    ...
    if (new_consent) {
        printf("Resuming Synora audio capture...\n");
        acr_start();
    } else {
        printf("Stopping Synora audio capture and purging cache...\n");
        acr_stop();
...
class MainActivity : Activity() {
    ...
    private fun initializeAcrSdk() {
        try {
            ...
            if (userConsented) {
                acrSdk.start()
                updateStatus("Synora Running (User Consented)")
            } else {
                updateStatus("Synora Stopped (User Opted Out)")
            }
            ...
            updateStatus("Error initializing Synora: ${e.message}")
            e.printStackTrace()
        }
    }
...
The Synora SDK exposes 6 core functions via C ABI. All are thread-safe unless otherwise noted.

### acr_init(acr_config_t *config)

**Purpose**: Initialize the Synora SDK. Must be called once before any other functions.
...
**Purpose**: Get the stable, anonymized device ID used by Synora for this device.
...
### 8.1 What Synora Collects

Synora collects only the minimum data required to provide audience measurement:
...
3. **IP Prefix (first 24 bits, not full IP)**
   ...
   - Full IP address is never sent to Synora servers
...
### 8.2 What Synora NEVER Collects
...
Consent requirements vary by regulation. Manufacturers must ensure proper consent before starting Synora.
...
Synora supports the following data subject rights:
...
Synora is designed for embedded TV devices with limited CPU and RAM. All measurements below are on a Samsung TV with ARM Cortex-A53 (1GHz), 1.5GB RAM.
...
**Conclusion**: Synora has negligible impact on TV performance. Your main TV UI will not stutter.
...
Synora provides a sandbox (non-production) environment for integration testing:
...
After starting Synora on your TV:
...
**Log output example**:
```
[ACR] 2026-04-10 14:23:45 Initializing Synora SDK v1.0.0
...
The Synora revenue model aligns incentives: you earn money when your devices generate valuable data insights.
...
- **Segment Price**: Synora auctions audience segments to advertisers. Prices vary by segment quality and demand (typical range: $0.02-$0.50 per data point)

Example:
- Device A captures 100,000 fingerprints in March
- Synora prices those fingerprints at average $0.10 per point
...
- **Tax**: Synora will issue Form 1099-NEC for US manufacturers (if >= $600 annual revenue)
...
**Symptoms**: TV feels sluggish, CPU monitor shows Synora using 5%+ CPU consistently.

**Causes**:
- `capture_interval_sec` too low (default 30 is fine; 5 would be high)
- Multiple Synora instances running (check for duplicate `acr_init()` calls)
...
The Synora SDK is distributed as a pre-compiled binary package via CDN:
...
Synora SDK uses semantic versioning: `MAJOR.MINOR.PATCH`
...
```cmake
# Find Synora SDK
find_package(Synora 1.2 REQUIRED)

# Link to your target
target_link_libraries(my_app PRIVATE Synora::acraas)
...
dependencies {
    // Synora SDK (available via Maven Central)
...
int main() {
    // Configure SDK
    acr_config_t cfg = {
        // Required: API key provided by Synora
    ...
    if (result != ACR_OK) {
        fprintf(stderr, "Failed to initialize Synora: %s\n", acr_error_string(result));
        return 1;
    }
    
    printf("Synora initialized successfully\n");
...
void on_user_changed_consent(bool new_consent_value) {
    ...
    if (!new_consent_value) {
        ...
        printf("User opted out. Synora is now inactive.\n");
    }
}
```
...
if (result != ACR_OK) {
    fprintf(stderr, "Failed to start Synora: %s\n", acr_error_string(result));
    return 1;
}

printf("Synora started. Capturing fingerprints...\n");
...
int main() {
    ...
    if (acr_init(&cfg) != ACR_OK) {
        fprintf(stderr, "Failed to init Synora\n");
        return 1;
    }
    ...
    // Step 5: Start
    if (acr_start() != ACR_OK) {
        fprintf(stderr, "Failed to start Synora\n");
        return 1;
    }
    
    printf("Synora running. Press Ctrl+C to stop.\n");
...
Synora requires explicit, informed user consent before any data collection. This is mandatory under:
...
bool get_acraas_consent() {
    ...
    // Check if "Synora" vendor has consent
...
void on_consent_changed(const char* new_consent_string) {
    ...
    if (!now_consented) {
        printf("Synora consent withdrawn. Data collection stopped.\n");
    }
}
```
...
# Expected output:
# [INFO] Synora initialized: device_id=abc123def456
...
**Error**: "Failed to initialize Synora: Network error"
...
Synora generates revenue by sharing anonymized audience data with advertisers and DSPs:
...
3. **Synora platform** generates revenue from these buys
